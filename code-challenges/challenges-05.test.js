...
const howMuchPencil = (str) => {
  ...
  for (let index = 0; index < str.length; index++) {
    newString = str.slice(index , str.length)
...
const listFoods = (recipe) => {
  ...
  recipe['ingredients'].forEach(element => {
    let ingredientsArr = element.slice(4,element.length).indexOf(' ');
    result.push(element.slice(ingredientsArr+5,element.length));
  });
  return result;
};
